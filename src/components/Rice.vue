<script>
  ...
  export default defineComponent({
    ...
    async mounted() {
      ...
      const info = (await axios.get(`https://rice-backend-node.entrydia7691.repl.co/info/${this.school_name}`));
      ...
      if (this.school_name == "포항제철중학교") {
        _res = (await axios.get(`https://rice-backend-node.entrydia7691.repl.co/meal/포항제철공업고등학교`)).data
      } else if (this.school_name == "중앙기독중학교") {
        _res = (await axios.get(`https://rice-backend-node.entrydia7691.repl.co/meal/중앙기독초등학교`)).data
      } else {
        _res = (await axios.get(`https://rice-backend-node.entrydia7691.repl.co/meal/${this.school_name}`)).data
      }
